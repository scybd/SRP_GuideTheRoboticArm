...
Mat FrameNumDetector::preprocess()
{
    // 相机图像预处理
	cvtColor(frame, frame, COLOR_BGR2HSV);
	Mat test = frame.clone();
	inRange(frame, Scalar(0, 0, 0), Scalar(179, 255, 130), frame);
	
	inRange(test, Scalar(0, 0, 0), Scalar(179, 255, 130), test);
...
void FrameNumDetector::getNum()
{
	if (!frameContours.empty() && !frameHierarchy.empty())
	{
		...
		for (int Index=0; Index<frameHierarchy.size(); Index++)
		{
			...
			// cout << tempRect.width << "--" << tempRect.height << endl;
			// 轮廓满足长宽比要求（数字外的方框的长宽比）
			if (tempRect.width>0.3*tempRect.height && tempRect.width<tempRect.height)
			{
				// 如果该轮廓有子轮廓且其子轮廓也有子轮廓
...
vector<NumObject> FrameNumDetector::allInOne(Mat& test)
{
    test = preprocess();
    getNum();
    getNormNum();
